...
func newWorkflowCmd(flags *rootFlags) *cobra.Command {
	...
	cmd.AddCommand(newWorkflowRecentSubscriptionUploadsCmd(flags))
...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  youtube-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  youtube-pp-cli workflow archive --full`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  youtube-pp-cli workflow status

  # Show status as JSON
  youtube-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("youtube-pp-cli")
			}

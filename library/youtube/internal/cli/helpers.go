...
func classifyAPIError(err error, flags *rootFlags) error {
	msg := err.Error()
	switch {
	// PATCH: Shared OAuth setup failures should exit as auth errors before any HTTP request is sent.
	case strings.Contains(msg, "OAuth token") || strings.Contains(msg, "OAuth client credentials"):
		classified := authErr(fmt.Errorf("%w\nhint: run 'youtube-pp-cli auth status' to inspect paths, then 'youtube-pp-cli auth login' to refresh the shared token.", err))
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'youtube-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your token. Set it with: youtube-pp-cli auth set-token <token>"+
			"\n      or: export YOUTUBE_DATA_OAUTH2C=<your-token>"+
			"\n      See API docs: https://google.com"+
			"\n      Run 'youtube-pp-cli doctor' to check auth status.", err))
...
func paginatedGet(c interface {
	...
	// PATCH: Preserve the API's real cursor parameter casing when generated
	// metadata lowercases it (YouTube requires pageToken, not pagetoken).
	cursorParam = canonicalParamName(params, cursorParam)
				...
				if nextCursorPath != "" {
					if tokenRaw, ok := rawAtPath(obj, nextCursorPath); ok {
						...
						if json.Unmarshal(tokenRaw, &token) == nil && token != "" {
							// PATCH: Stop before re-fetching the same logical
							// page forever when an API ignores the emitted
							// cursor parameter or echoes a stuck cursor.
							if token == lastCursor {
								return nil, fmt.Errorf("pagination cursor did not advance for %s: %s returned %q again; check cursor parameter %q", path, nextCursorPath, token, cursorParam)
							}
							lastCursor = token
...
func canonicalParamName(params map[string]string, name string) string {
	if name == "" {
		return name
	}

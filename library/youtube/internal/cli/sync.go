...
Exit codes & warnings:
  ...
  youtube-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  youtube-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  youtube-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  youtube-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  youtube-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/youtube-pp-cli/data.db)")
...
func determinePaginationDefaults() paginationDefaults {
	// PATCH: YouTube query parameter names are case-sensitive.

...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "youtube-pp-cli", "http")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	// PATCH: Dry runs preview cached auth only; live calls load and refresh
	// the shared OAuth token without first populating Config.AccessToken with
	// a possibly stale token-file value.
	authHeader := ""
	if c.DryRun {
		if c.Config != nil {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "youtube-pp-cli/v3")
		}
		...
		// PATCH: Google access tokens can be revoked or otherwise rejected
		// before their stored expiry. On the first 401 from a token-file
		// credential, force a refresh and retry once before surfacing the
		// authentication error.
		if resp.StatusCode == http.StatusUnauthorized && !forcedAuthRefresh && attempt < maxRetries {
			refreshedHeader, refreshed, refreshErr := c.forceRefreshAuthHeader()
			if refreshErr == nil && refreshed {
				authHeader = refreshedHeader
				forcedAuthRefresh = true
				lastErr = apiErr
				continue
			}
		}
...
func (c *Client) authHeader() (string, error) {
	...
	if header := c.Config.ManualAuthHeader(); header != "" {
		return header, nil
	}

	// PATCH: Use the official OAuth token source so expired shared tokens refresh automatically.
	token, _, err := c.Config.FreshOAuthToken(context.Background(), c.HTTPClient, config.DefaultYoutubeScopes())
	if err != nil {
		return "", err
	}
	if token == nil || token.AccessToken == "" {
		return "", fmt.Errorf("OAuth token at %s has no access_token", c.Config.TokenPath)
	}
...
func (c *Client) forceRefreshAuthHeader() (string, bool, error) {
	if c.Config == nil {
		return "", false, nil
	}
	if c.Config.AuthHeaderVal != "" || c.Config.YoutubeDataOauth2c != "" {
		return "", false, nil
	}
	if status := c.Config.TokenFileStatus(); !status.Exists {
		return "", false, nil
	}
	token, _, err := c.Config.ForceFreshOAuthToken(context.Background(), c.HTTPClient, config.DefaultYoutubeScopes())
	if err != nil {
		return "", true, err
	}
	if token == nil || token.AccessToken == "" {
		return "", true, fmt.Errorf("OAuth token at %s has no access_token", c.Config.TokenPath)
	}
	tokenType := token.TokenType
	if tokenType == "" {
		tokenType = "Bearer"
	}
	return tokenType + " " + token.AccessToken, true, nil
}

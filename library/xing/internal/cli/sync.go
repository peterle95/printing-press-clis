...
Exit codes & warnings:
  ...
  xing-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  xing-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  xing-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  xing-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  xing-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/xing-pp-cli/data.db)")

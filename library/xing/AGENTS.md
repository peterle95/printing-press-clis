# Xing Printed CLI Agent Guide

This directory is a generated `xing-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
xing-pp-cli doctor --json
xing-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
xing-pp-cli which "<capability>" --json
xing-pp-cli <command> --help
```
...
```bash
xing-pp-cli <command> --help
xing-pp-cli <command> --dry-run --agent
```

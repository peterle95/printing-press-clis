...
Exit codes & warnings:
  ...
  getinit-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  getinit-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  getinit-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  getinit-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  getinit-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/getinit-pp-cli/data.db)")

...
Exit codes & warnings:
  ...
  remotive-jobs-pp-cli sync

  # Sync specific resources only
  remotive-jobs-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  remotive-jobs-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  remotive-jobs-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  remotive-jobs-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  remotive-jobs-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/remotive-jobs-pp-cli/data.db)")

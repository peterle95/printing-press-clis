...
Exit codes & warnings:
  ...
  stepstone-pp-cli sync

  # Sync specific resources only
  stepstone-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  stepstone-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  stepstone-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  stepstone-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  stepstone-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/stepstone-pp-cli/data.db)")

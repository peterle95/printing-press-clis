...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BA_JOBSUCHE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BA_JOBSUCHE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "ba-jobsuche-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.ba-jobsuche-pp-cli/feedback.jsonl.
When ` + "`BA_JOBSUCHE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BA_JOBSUCHE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ba-jobsuche-pp-cli feedback list
  ba-jobsuche-pp-cli feedback list --limit 5
  ba-jobsuche-pp-cli feedback list --json`,

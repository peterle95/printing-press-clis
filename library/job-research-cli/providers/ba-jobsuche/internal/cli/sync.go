...
Exit codes & warnings:
  ...
  ba-jobsuche-pp-cli sync

  # Sync specific resources only
  ba-jobsuche-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  ba-jobsuche-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  ba-jobsuche-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  ba-jobsuche-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  ba-jobsuche-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/ba-jobsuche-pp-cli/data.db)")

...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("XING_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("XING_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "xing-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.xing-pp-cli/feedback.jsonl.
When ` + "`XING_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`XING_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  xing-pp-cli feedback list
  xing-pp-cli feedback list --limit 5
  xing-pp-cli feedback list --json`,

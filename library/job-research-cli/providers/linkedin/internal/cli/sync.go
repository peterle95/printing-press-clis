...
Exit codes & warnings:
  ...
  linkedin-pp-cli sync

  # Sync specific resources only
  linkedin-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  linkedin-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  linkedin-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  linkedin-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  linkedin-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/linkedin-pp-cli/data.db)")

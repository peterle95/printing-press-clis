...
Exit codes & warnings:
  ...
  berlinstartupjobs-pp-cli sync

  # Sync specific resources only
  berlinstartupjobs-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  berlinstartupjobs-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  berlinstartupjobs-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  berlinstartupjobs-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  berlinstartupjobs-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/berlinstartupjobs-pp-cli/data.db)")

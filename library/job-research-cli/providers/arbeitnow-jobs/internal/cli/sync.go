...
Exit codes & warnings:
  ...
  arbeitnow-jobs-pp-cli sync

  # Sync specific resources only
  arbeitnow-jobs-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  arbeitnow-jobs-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  arbeitnow-jobs-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  arbeitnow-jobs-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  arbeitnow-jobs-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/arbeitnow-jobs-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "job-board-api":
		return db.UpsertJobBoardApi(data)
	default:

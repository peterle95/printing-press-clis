...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GERMANTECHJOBS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GERMANTECHJOBS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "germantechjobs-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.germantechjobs-pp-cli/feedback.jsonl.
When ` + "`GERMANTECHJOBS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GERMANTECHJOBS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  germantechjobs-pp-cli feedback list
  germantechjobs-pp-cli feedback list --limit 5
  germantechjobs-pp-cli feedback list --json`,

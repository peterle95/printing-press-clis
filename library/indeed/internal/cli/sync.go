...
Exit codes & warnings:
  ...
  indeed-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  indeed-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  indeed-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  indeed-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  indeed-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/indeed-pp-cli/data.db)")
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"indeed-public-surface-jobs": "/jobs",
	}

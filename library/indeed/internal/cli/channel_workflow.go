...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  indeed-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  indeed-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"indeed-public-surface-jobs"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  indeed-pp-cli workflow status

  # Show status as JSON
  indeed-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("indeed-pp-cli")
			}

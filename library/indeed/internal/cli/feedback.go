...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".indeed-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("INDEED_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("INDEED_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "indeed-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.indeed-pp-cli/feedback.jsonl.
When ` + "`INDEED_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`INDEED_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  indeed-pp-cli feedback list
  indeed-pp-cli feedback list --limit 5
  indeed-pp-cli feedback list --json`,

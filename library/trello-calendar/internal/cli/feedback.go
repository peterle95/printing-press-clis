...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".trello-calendar-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TRELLO_CALENDAR_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TRELLO_CALENDAR_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "trello-calendar-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.trello-calendar-pp-cli/feedback.jsonl.
When ` + "`TRELLO_CALENDAR_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TRELLO_CALENDAR_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  trello-calendar-pp-cli feedback list
  trello-calendar-pp-cli feedback list --limit 5
  trello-calendar-pp-cli feedback list --json`,

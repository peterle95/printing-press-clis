...
In local mode: searches locally synced data only.`,
  ...
  trello-calendar-pp-cli search "error timeout"

  # Force local search only
  trello-calendar-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  trello-calendar-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  trello-calendar-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'trello-calendar-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()

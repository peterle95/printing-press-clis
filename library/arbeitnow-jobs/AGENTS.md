# Arbeitnow Jobs Printed CLI Agent Guide

This directory is a generated `arbeitnow-jobs-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
arbeitnow-jobs-pp-cli doctor --json
arbeitnow-jobs-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
arbeitnow-jobs-pp-cli which "<capability>" --json
arbeitnow-jobs-pp-cli <command> --help
```
...
```bash
arbeitnow-jobs-pp-cli <command> --agent
```
...
```bash
arbeitnow-jobs-pp-cli <command> --help
arbeitnow-jobs-pp-cli <command> --dry-run --agent
```

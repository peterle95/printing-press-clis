...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ARBEITNOW_JOBS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ARBEITNOW_JOBS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "arbeitnow-jobs-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.arbeitnow-jobs-pp-cli/feedback.jsonl.
When ` + "`ARBEITNOW_JOBS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ARBEITNOW_JOBS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  arbeitnow-jobs-pp-cli feedback list
  arbeitnow-jobs-pp-cli feedback list --limit 5
  arbeitnow-jobs-pp-cli feedback list --json`,

...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  arbeitnow-jobs-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  arbeitnow-jobs-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"job-board-api"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  arbeitnow-jobs-pp-cli workflow status

  # Show status as JSON
  arbeitnow-jobs-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("arbeitnow-jobs-pp-cli")
			}

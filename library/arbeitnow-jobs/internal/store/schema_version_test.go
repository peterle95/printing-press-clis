...
// TestMigrate_AddsColumnsOnUpgrade_JobBoardApi verifies that opening a
...
func TestMigrate_AddsColumnsOnUpgrade_JobBoardApi(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "job_board_api" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	rows, err := s.DB().Query(`PRAGMA table_info("job_board_api")`)
	if err != nil {
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from job_board_api after migrate", want)
		}
	}
}

...
// TestUpsertBatch_PopulatesJobBoardApiTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed job_board_api table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesJobBoardApiTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("job-board-api", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "job-board-api").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "job_board_api")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count job_board_api: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("job_board_api count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

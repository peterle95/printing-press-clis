...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GETINIT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GETINIT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "getinit-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.getinit-pp-cli/feedback.jsonl.
When ` + "`GETINIT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GETINIT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  getinit-pp-cli feedback list
  getinit-pp-cli feedback list --limit 5
  getinit-pp-cli feedback list --json`,

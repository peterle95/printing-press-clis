...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  getinit-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  getinit-pp-cli workflow archive --full`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  getinit-pp-cli workflow status

  # Show status as JSON
  getinit-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("getinit-pp-cli")
			}

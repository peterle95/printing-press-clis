# Getinit Printed CLI Agent Guide

This directory is a generated `getinit-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
getinit-pp-cli doctor --json
getinit-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
getinit-pp-cli which "<capability>" --json
getinit-pp-cli <command> --help
```
...
```bash
getinit-pp-cli <command> --help
getinit-pp-cli <command> --dry-run --agent
```

# Germantechjobs Printed CLI Agent Guide

This directory is a generated `germantechjobs-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
germantechjobs-pp-cli doctor --json
germantechjobs-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
germantechjobs-pp-cli which "<capability>" --json
germantechjobs-pp-cli <command> --help
```
...
```bash
germantechjobs-pp-cli <command> --agent
```
...
```bash
germantechjobs-pp-cli <command> --help
germantechjobs-pp-cli <command> --dry-run --agent
```

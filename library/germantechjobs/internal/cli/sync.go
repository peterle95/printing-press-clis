...
Exit codes & warnings:
  ...
  germantechjobs-pp-cli sync

  # Sync specific resources only
  germantechjobs-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  germantechjobs-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  germantechjobs-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  germantechjobs-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  germantechjobs-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/germantechjobs-pp-cli/data.db)")

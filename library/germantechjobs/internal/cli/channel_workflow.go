...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  germantechjobs-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  germantechjobs-pp-cli workflow archive --full`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  germantechjobs-pp-cli workflow status

  # Show status as JSON
  germantechjobs-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("germantechjobs-pp-cli")
			}

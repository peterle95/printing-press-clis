...
func TestUpsertBatch_PopulatesPageTable(t *testing.T) {
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "test-001", "skill_areas_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-002", "skill_areas_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-003", "skill_areas_id": "test-parent-001"}`),
	}
	if _, _, err := s.UpsertBatch("page", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}

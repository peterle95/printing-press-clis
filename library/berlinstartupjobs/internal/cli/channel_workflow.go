...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  berlinstartupjobs-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  berlinstartupjobs-pp-cli workflow archive --full`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  berlinstartupjobs-pp-cli workflow status

  # Show status as JSON
  berlinstartupjobs-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("berlinstartupjobs-pp-cli")
			}

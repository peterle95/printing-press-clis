...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BERLINSTARTUPJOBS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BERLINSTARTUPJOBS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "berlinstartupjobs-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.berlinstartupjobs-pp-cli/feedback.jsonl.
When ` + "`BERLINSTARTUPJOBS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BERLINSTARTUPJOBS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  berlinstartupjobs-pp-cli feedback list
  berlinstartupjobs-pp-cli feedback list --limit 5
  berlinstartupjobs-pp-cli feedback list --json`,

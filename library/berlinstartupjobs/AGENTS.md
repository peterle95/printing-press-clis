# Berlinstartupjobs Printed CLI Agent Guide

This directory is a generated `berlinstartupjobs-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
berlinstartupjobs-pp-cli doctor --json
berlinstartupjobs-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
berlinstartupjobs-pp-cli which "<capability>" --json
berlinstartupjobs-pp-cli <command> --help
```
...
```bash
berlinstartupjobs-pp-cli <command> --agent
```
...
```bash
berlinstartupjobs-pp-cli <command> --help
berlinstartupjobs-pp-cli <command> --dry-run --agent
```

...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("REMOTIVE_JOBS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("REMOTIVE_JOBS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "remotive-jobs-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.remotive-jobs-pp-cli/feedback.jsonl.
When ` + "`REMOTIVE_JOBS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`REMOTIVE_JOBS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  remotive-jobs-pp-cli feedback list
  remotive-jobs-pp-cli feedback list --limit 5
  remotive-jobs-pp-cli feedback list --json`,

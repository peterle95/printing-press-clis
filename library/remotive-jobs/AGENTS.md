# Remotive Jobs Printed CLI Agent Guide

This directory is a generated `remotive-jobs-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
remotive-jobs-pp-cli doctor --json
remotive-jobs-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
remotive-jobs-pp-cli which "<capability>" --json
remotive-jobs-pp-cli <command> --help
```
...
```bash
remotive-jobs-pp-cli <command> --agent
```
...
```bash
remotive-jobs-pp-cli <command> --help
remotive-jobs-pp-cli <command> --dry-run --agent
```

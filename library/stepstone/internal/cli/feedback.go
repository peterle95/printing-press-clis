...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("STEPSTONE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("STEPSTONE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "stepstone-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.stepstone-pp-cli/feedback.jsonl.
When ` + "`STEPSTONE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`STEPSTONE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  stepstone-pp-cli feedback list
  stepstone-pp-cli feedback list --limit 5
  stepstone-pp-cli feedback list --json`,

# Stepstone Printed CLI Agent Guide

This directory is a generated `stepstone-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
stepstone-pp-cli doctor --json
stepstone-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
stepstone-pp-cli which "<capability>" --json
stepstone-pp-cli <command> --help
```
...
```bash
stepstone-pp-cli <command> --agent
```
...
```bash
stepstone-pp-cli <command> --help
stepstone-pp-cli <command> --dry-run --agent
```

...
var readCommandResources = map[string][]string{
	"airbnb-pp-cli airbnb_wishlist":        {"airbnb_wishlist"},
	"airbnb-pp-cli airbnb_wishlist list":   {"airbnb_wishlist"},
	"airbnb-pp-cli airbnb_wishlist get":    {"airbnb_wishlist"},
	"airbnb-pp-cli airbnb_wishlist search": {"airbnb_wishlist"},
}
...
func cachePolicy() cliutil.Policy {
	...
	envOptOut := "AIRBNB_PP_NO_AUTO_REFRESH"
...
func autoRefreshIfStale(ctx context.Context, flags *rootFlags, resources []string) (meta cliutil.FreshnessMeta) {
	...
	if err := runAutoRefresh(refreshCtx, flags, db, resources); err != nil {
		fmt.Fprintf(os.Stderr, "warning: using stale airbnb-pp-cli cache (refresh failed: %v)\n", err)

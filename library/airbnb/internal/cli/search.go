...
In local mode: searches locally synced data only.`,
  ...
  airbnb-pp-cli search "error timeout"

  # Force local search only
  airbnb-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  airbnb-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  airbnb-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /graphql
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post("/graphql", map[string]any{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'airbnb-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()

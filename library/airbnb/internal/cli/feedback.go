...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".airbnb-pp-cli")
	if err := os.MkdirAll(dir, 0o700); err != nil {
		return "", fmt.Errorf("creating state dir: %w", err)
	}
	return filepath.Join(dir, "feedback.jsonl"), nil
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AIRBNB_PP_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AIRBNB_PP_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "airbnb-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.airbnb-pp-cli/feedback.jsonl.
When ` + "`AIRBNB_PP_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AIRBNB_PP_FEEDBACK_AUTO_SEND=true`" + `, the entry is

...
profile by name when the installed backend supports it.`,
		Example: `  airbnb-pp-cli auth login --chrome
  airbnb-pp-cli auth login --browser
  airbnb-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if !browserFlag {
				fmt.Fprintln(cmd.OutOrStdout(), "Use --chrome or --browser to authenticate from your browser session:")
				fmt.Fprintf(cmd.OutOrStdout(), "  airbnb-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  airbnb-pp-cli auth login --browser\n")
				return nil
			}
			...
			if err := saveAirbnbCookieJSON(cookies); err != nil {
				return configErr(fmt.Errorf("saving cookies.json: %w", err))
			}
...
func saveAirbnbCookieJSON(cookies string) error {
	home, err := os.UserHomeDir()
	if err != nil {
		return err
	}
	dir := filepath.Join(home, ".config", "airbnb-pp-cli")
	if err := os.MkdirAll(dir, 0o700); err != nil {
		return err
	}
	data, err := json.MarshalIndent(map[string]string{"cookies": cookies}, "", "  ")
	if err != nil {
		return err
	}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  airbnb-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if header == "" {
				...
				fmt.Fprintf(w, "  airbnb-pp-cli auth login --chrome\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}

			fmt.Fprintln(w, green("Authenticated"))
			fmt.Fprintf(w, "  Source: %s\n", cfg.AuthSource)
			fmt.Fprintf(w, "  Domain: .airbnb.com\n")
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  airbnb-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)

...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  airbnb-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  airbnb-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"airbnb_wishlist"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  airbnb-pp-cli workflow status

  # Show status as JSON
  airbnb-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("airbnb-pp-cli")
			}

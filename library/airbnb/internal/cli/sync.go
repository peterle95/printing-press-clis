...
Exit codes & warnings:
  ...
  airbnb-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  airbnb-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  airbnb-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  airbnb-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  airbnb-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/airbnb-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "airbnb_listing":
		return db.UpsertAirbnbListing(data)
	case "airbnb_wishlist":
		return db.UpsertAirbnbWishlist(data)
	case "host":
		return db.UpsertHost(data)
	default:
...
func syncResourcePath(resource string) string {
	paths := map[string]string{
		"airbnb_wishlist": "/api/v3/WishlistIndexPageQuery/b8b421d802c399b55fb6ac1111014807a454184ad38f198365beb7836c018c18",
	}

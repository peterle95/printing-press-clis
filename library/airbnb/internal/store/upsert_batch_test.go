...
// TestUpsertBatch_PopulatesAirbnbListingTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed airbnb_listing table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesAirbnbListingTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("airbnb_listing", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "airbnb_listing").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "airbnb_listing")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count airbnb_listing: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("airbnb_listing count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_PopulatesAirbnbWishlistTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed airbnb_wishlist table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesAirbnbWishlistTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("airbnb_wishlist", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "airbnb_wishlist").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "airbnb_wishlist")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count airbnb_wishlist: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("airbnb_wishlist count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_PopulatesHostTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed host table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesHostTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("host", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "host").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "host")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count host: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("host count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

# Ba Jobsuche Printed CLI Agent Guide

This directory is a generated `ba-jobsuche-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
ba-jobsuche-pp-cli doctor --json
ba-jobsuche-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
ba-jobsuche-pp-cli which "<capability>" --json
ba-jobsuche-pp-cli <command> --help
```
...
```bash
ba-jobsuche-pp-cli <command> --agent
```
...
```bash
ba-jobsuche-pp-cli <command> --help
ba-jobsuche-pp-cli <command> --dry-run --agent
```

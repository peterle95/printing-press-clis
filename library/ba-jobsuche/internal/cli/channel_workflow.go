...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  ba-jobsuche-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  ba-jobsuche-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"ba-jobsuche-jobs"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  ba-jobsuche-pp-cli workflow status

  # Show status as JSON
  ba-jobsuche-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("ba-jobsuche-pp-cli")
			}

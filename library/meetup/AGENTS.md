# Meetup Printed CLI Agent Guide

This directory is a generated `meetup-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
meetup-pp-cli doctor --json
meetup-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
meetup-pp-cli which "<capability>" --json
meetup-pp-cli <command> --help
```
...
```bash
meetup-pp-cli <command> --agent
```

When presenting event results to the user, always include the direct Meetup.com `url` link so they can RSVP or view details.

Before running an unfamiliar command that may mutate remote state, inspect its help and prefer a dry run:

```bash
meetup-pp-cli <command> --help
meetup-pp-cli <command> --dry-run --agent
```

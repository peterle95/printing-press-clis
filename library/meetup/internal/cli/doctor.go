...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  meetup-pp-cli doctor
  meetup-pp-cli doctor --json
  meetup-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					report["auth"] = "optional (not configured; public event search works without a token)"
					report["auth_hint"] = "Set MEETUP_GRAPHQL_SEARCH_BEARER_AUTH only for account-specific GraphQL fields."
				} else {
					authConfigured = true
			...
			if os.Getenv("MEETUP_GRAPHQL_SEARCH_BEARER_AUTH") != "" {
				authEnvSet = append(authEnvSet, "MEETUP_GRAPHQL_SEARCH_BEARER_AUTH")
				...
				authEnvInfo = append(authEnvInfo, "optional credential not set; public event search is available")
			}
			switch {

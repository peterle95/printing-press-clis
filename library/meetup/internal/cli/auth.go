...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Meetup Graphql Search",
	}
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  meetup-pp-cli auth setup\n  meetup-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "No setup URL is configured for this CLI; check the API's docs.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export MEETUP_GRAPHQL_SEARCH_BEARER_AUTH=\"<your-token>\"")
			fmt.Fprintln(w, "  meetup-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  meetup-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export MEETUP_GRAPHQL_SEARCH_BEARER_AUTH=\"your-token-here\"")
				fmt.Fprintf(w, "  meetup-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  meetup-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  meetup-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("MEETUP_GRAPHQL_SEARCH_BEARER_AUTH") != "" {
				envStillSet = "MEETUP_GRAPHQL_SEARCH_BEARER_AUTH"
			}

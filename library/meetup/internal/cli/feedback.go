...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".meetup-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MEETUP_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MEETUP_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "meetup-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.meetup-pp-cli/feedback.jsonl.
When ` + "`MEETUP_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MEETUP_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  meetup-pp-cli feedback list
  meetup-pp-cli feedback list --limit 5
  meetup-pp-cli feedback list --json`,

...
// TestMigrate_AddsColumnsOnUpgrade_PatientHealthSearch verifies that opening a
...
func TestMigrate_AddsColumnsOnUpgrade_PatientHealthSearch(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "patient_health_search" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	rows, err := s.DB().Query(`PRAGMA table_info("patient_health_search")`)
	if err != nil {
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from patient_health_search after migrate", want)
		}
	}
}

...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".doctolib-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DOCTOLIB_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DOCTOLIB_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "doctolib-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.doctolib-pp-cli/feedback.jsonl.
When ` + "`DOCTOLIB_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DOCTOLIB_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  doctolib-pp-cli feedback list
  doctolib-pp-cli feedback list --limit 5
  doctolib-pp-cli feedback list --json`,

...
Exit codes & warnings:
  ...
  doctolib-pp-cli sync

  # Sync specific resources only
  doctolib-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  doctolib-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  doctolib-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  doctolib-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  doctolib-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/doctolib-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "patient-health-search":
		return db.UpsertPatientHealthSearch(data)
	default:

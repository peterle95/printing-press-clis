...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ENGLISHJOBS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ENGLISHJOBS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "englishjobs-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.englishjobs-pp-cli/feedback.jsonl.
When ` + "`ENGLISHJOBS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ENGLISHJOBS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  englishjobs-pp-cli feedback list
  englishjobs-pp-cli feedback list --limit 5
  englishjobs-pp-cli feedback list --json`,

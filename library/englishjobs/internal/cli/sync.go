...
Exit codes & warnings:
  ...
  englishjobs-pp-cli sync

  # Sync specific resources only
  englishjobs-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  englishjobs-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  englishjobs-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  englishjobs-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  englishjobs-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/englishjobs-pp-cli/data.db)")

# Englishjobs Printed CLI Agent Guide

This directory is a generated `englishjobs-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
englishjobs-pp-cli doctor --json
englishjobs-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
englishjobs-pp-cli which "<capability>" --json
englishjobs-pp-cli <command> --help
```
...
```bash
englishjobs-pp-cli <command> --agent
```
...
```bash
englishjobs-pp-cli <command> --help
englishjobs-pp-cli <command> --dry-run --agent
```

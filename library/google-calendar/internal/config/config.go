...
type Config struct {
	...
	Path            string `toml:"-"`
	CalendarOauth2c string `toml:"oauth2c"`
}
...
func Default(configPath string) *Config {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("GOOGLE_CALENDAR_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "google-calendar-pp-cli", "config.toml")
	}
	cfg.Path = path
	return cfg
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("CALENDAR_OAUTH2C"); v != "" {
		cfg.CalendarOauth2c = v
		cfg.AuthSource = "env:CALENDAR_OAUTH2C"
	}
	...
	if cfg.AuthSource == "" && cfg.CalendarOauth2c != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("GOOGLE_CALENDAR_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	if v := os.Getenv("GOOGLE_CALENDAR_AUTHORIZATION_URL"); v != "" {
		cfg.AuthorizationURL = v
	}
	if v := os.Getenv("GOOGLE_CALENDAR_TOKEN_URL"); v != "" {
		cfg.TokenURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.CalendarOauth2c != "" {
		c.AuthSource = "env:CALENDAR_OAUTH2C"
		return "Bearer " + c.CalendarOauth2c
	}
	if c.AccessToken != "" {
		c.AuthSource = "oauth2"

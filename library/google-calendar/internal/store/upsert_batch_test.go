...
// TestUpsertBatch_PopulatesAclTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed acl table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesAclTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("acl", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "acl").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "acl")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count acl: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("acl count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}
...
// dispatches paginated items into both the generic resources table AND the
// typed free_busy table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesFreeBusyTable(t *testing.T) {
	...
	if _, _, err := s.UpsertBatch("free-busy", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "free-busy").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "free_busy")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count free_busy: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("free_busy count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".google-calendar-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GOOGLE_CALENDAR_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GOOGLE_CALENDAR_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "google-calendar-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.google-calendar-pp-cli/feedback.jsonl.
When ` + "`GOOGLE_CALENDAR_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GOOGLE_CALENDAR_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  google-calendar-pp-cli feedback list
  google-calendar-pp-cli feedback list --limit 5
  google-calendar-pp-cli feedback list --json`,

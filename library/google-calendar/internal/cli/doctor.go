...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  google-calendar-pp-cli doctor
  google-calendar-pp-cli doctor --json
  google-calendar-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export CALENDAR_OAUTH2C=<your-key>"
					report["auth_docs_url"] = "https://google.com"
				} else {
					authConfigured = true
			...
			if os.Getenv("CALENDAR_OAUTH2C") != "" {
				authEnvSet = append(authEnvSet, "CALENDAR_OAUTH2C")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "CALENDAR_OAUTH2C")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'google-calendar-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'google-calendar-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'google-calendar-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'google-calendar-pp-cli sync' to refresh."
	}
	return report
}

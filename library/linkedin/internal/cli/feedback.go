...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("LINKEDIN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("LINKEDIN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "linkedin-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.linkedin-pp-cli/feedback.jsonl.
When ` + "`LINKEDIN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`LINKEDIN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  linkedin-pp-cli feedback list
  linkedin-pp-cli feedback list --limit 5
  linkedin-pp-cli feedback list --json`,

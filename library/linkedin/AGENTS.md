# Linkedin Printed CLI Agent Guide

This directory is a generated `linkedin-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
linkedin-pp-cli doctor --json
linkedin-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
linkedin-pp-cli which "<capability>" --json
linkedin-pp-cli <command> --help
```
...
```bash
linkedin-pp-cli <command> --help
linkedin-pp-cli <command> --dry-run --agent
```
